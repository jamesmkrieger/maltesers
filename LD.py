''' module for use in pymol to calculate upper lobe and lower lobe contact numbers
 ...
 LD pdbcode,chain1,chain2,lobe, reverse
...
0 means don't reverse the order e.g. LD 3saj,A,C,UL is fine
...
def LD(pdbcode,chain1,chain2,lobe,reversechains=0):
	...
	fo = open('Documents/pdbs/' + lobe + '_IF_' + pdbcode + '_' + chain1 + '_LD','w')
	for line in fi:
		if line.find('ATOM') == 0:
			resid = line.split()[5]
			atomName = line.split()[2]
			if int(reversechains) == 0:
				currCount = cmd.count_atoms(lobe + "_IF_" + pdbcode + "_" + chain1 + " within 12 of /" + pdbcode + "_" + chain1 + chain2 + "//" + chain1 + "/" + resid + "/" + atomName)
			elif int(reversechains) == 1:
                                currCount = cmd.count_atoms(lobe + "_IF_" + pdbcode + "_" + chain1 + " within 12 of /" + pdbcode + "_" + chain2 + chain1 + "//" + chain1 + "/" + resid + "/" + atomName)
			elif int(reversechains) == -1:
				currCount = cmd.count_atoms(lobe + "_IF_" + pdbcode + "_" + chain1 + " within 12 of /" + pdbcode + "//" + chain1 + "/" + resid + "/" + atomName)
